        ...
        @forelse($runs as $run)
            ...
            @if($run->status==='completed')
                <tr><td colspan="5" class="bg-body-tertiary"><div class="table-responsive"><table class="table table-sm mb-0">
                    <thead><tr><th>{{__('hr.employee')}}</th><th>{{__('hr.gross')}}</th><th>OT</th><th>{{__('hr.ssc_employee')}}</th><th>{{__('hr.income_tax')}}</th><th>{{__('hr.surcharge')}}</th><th>{{__('hr.net')}}</th><th></th></tr></thead>
                    <tbody>@foreach($run->payslips as $p)<tr><td>{{$p->employee->name}}</td><td class="text-jod">{{$p->gross_salary}}</td><td class="text-jod">{{$p->overtime_pay}}</td><td class="text-jod">{{$p->ssc_employee}}</td><td class="text-jod">{{$p->income_tax}}</td><td class="text-jod">{{$p->surcharge}}</td><td class="text-jod"><strong>{{$p->net_salary}}</strong></td><td><a href="{{route('payslips.pdf',$p)}}">PDF</a></td></tr>@endforeach</tbody>
                </table></div></td></tr>
            @endif
