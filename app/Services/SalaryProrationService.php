...
class SalaryProrationService
{
    ...
    public function calculate(Employee $employee, CarbonInterface $periodStart, CarbonInterface $periodEnd, array $settings): array
    {
        ...
        $fullPeriod = $employmentStart->isSameDay($periodStart)
            && $employmentEnd->isSameDay($periodEnd)
            && $changes->isEmpty();
        ...
        if ($fullPeriod) {
            ...
            return [
                'contract_salary' => Money::round($contractSalary),
                'payable_salary' => Money::round($startSalary),
                'proration_adjustment' => Money::round($contractSalary->minus($startSalary)),
                'daily_rate' => Money::round($startSalary->dividedBy($divisor, 12, RoundingMode::HALF_UP)),
                'payable_days' => $days,
                'employment_start' => $employmentStart->toDateString(),
                'employment_end' => $employmentEnd->toDateString(),
                'prorated' => false,
                'salary_segments' => [[
                    'start' => $employmentStart->toDateString(),
                    'end' => $employmentEnd->toDateString(),
                    'monthly_salary' => Money::round($startSalary),
                    'days' => $days,
                ]],
            ];
        }
        ...
        foreach ($changes as $change) {
            $boundaries->push(['date' => $change->effective_from->copy(), 'salary' => Money::d($change->amount)]);
        }
        ...
        foreach ($boundaries->values() as $index => $boundary) {
            ...
            if ($segmentEnd->gt($employmentEnd)) {
                $segmentEnd = $employmentEnd->copy();
            }
            ...
            $daysTotal += $days;
            $daily = $boundary['salary']->dividedBy($divisor, 12, RoundingMode::HALF_UP);
            $amount = $daily->multipliedBy((string) $days);
            $payable = $payable->plus($amount);
            $segments[] = [
                'start' => $segmentStart->toDateString(),
                'end' => $segmentEnd->toDateString(),
                'monthly_salary' => Money::round($boundary['salary']),
        ...
        return [
            ...
            'proration_adjustment' => Money::round($contractSalary->minus($payable)),
            'daily_rate' => Money::round($contractSalary->dividedBy($divisor, 12, RoundingMode::HALF_UP)),
            'payable_days' => $daysTotal,
            'employment_start' => $employmentStart->toDateString(),
            'employment_end' => $employmentEnd->toDateString(),
            'prorated' => true,
